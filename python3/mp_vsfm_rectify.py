...
def generate_medium_texture(files, img_path, contactimg_file, reorder = None, skip = []):
	...
	x_imgs_per_tile = int(grid_w / 2.0)
	y_imgs_per_tile = int(grid_h / 2.0)
	print("Images per tile", x_imgs_per_tile, "x", y_imgs_per_tile)
	...
	tile_w = int(thumb_w * x_imgs_per_tile)
	tile_h = int(thumb_h * y_imgs_per_tile)
	print("Adjusted output texture is", tile_w, "x", tile_h)
	...
	for ty in range(y_tiles):
		for tx in range(x_tiles):
			tilenum = tx + ty * x_tiles
			print("generating tile", tilenum, "(", tx, ty,"):", end=", ")
...
def generate_large_texture(files, img_path, contactimg_file, reorder = None, skip = []):
	"""Generate medium tiled texture, 2x2 <= max_texture_dimension"""
	global max_texture_size, grid_h, grid_w, num_textures, thumb_w, thumb_h, x_tiles, y_tiles, x_imgs_per_tile, y_imgs_per_tile
	...
	cam_img = cv2.imread(img_file)
	img_h, img_w, img_chan = cam_img.shape
	print("Input images are", img_w, "x", img_h)

	total_w = grid_w * img_w
	total_h = grid_h * img_h
	print("Full res texture is", total_w, "x", total_h)
	
	x_imgs_per_tile = math.ceil(grid_w / 3.0)
	y_imgs_per_tile = math.ceil(grid_h / 3.0)
	print("Images per tile", x_imgs_per_tile, "x", y_imgs_per_tile)
	...
	# 	tile_h = max_texture_size
	# 	tile_w = math.floor(float(max_texture_size) / float(img_w)) * img_w	
	# print("Output tile textures are", tile_w, "x", tile_h)

	if x_imgs_per_tile * img_w > y_imgs_per_tile * img_h:
		tile_w = max_texture_size
		tile_h = math.floor(float(max_texture_size)/float(x_imgs_per_tile*img_w) \
			* (y_imgs_per_tile*img_h))
	else:
		tile_h = max_texture_size
		tile_w = math.floor(float(max_texture_size)/float(y_imgs_per_tile*img_h) \
			* (x_imgs_per_tile*img_w))
	print("Output tile textures are", tile_w, "x", tile_h)
	...
	for ty in range(y_tiles):
		for tx in range(x_tiles):
			...
			print("writing contact image", tile_img.shape, "to", this_texture_file)#os.path.basename(this_texture_file))
			cv2.imwrite(this_texture_file, tile_img)
...
def write_xml_scene(xmlfile, texture_files, image_files, camresults, reorder = None, skip = [], do_inverty=False):
	...
	print("Writing scene descriptor file", xmlfile)
	out_f = open(xmlfile, 'w')
...
if __name__ == '__main__':
	...
	parser.add_argument('--large', dest='dolarge', action='store_true', help='generate a large size tiled texture (3x3)')
	...
	print("do medium sized tiled textures?", do_medium)
	print("do large sized tiled textures?", do_large)
	...
	if do_laptop:
		...
		texturepath = os.path.join(texroot, "large")    
	else:
		texturepath = texroot
		
	xmlfile = os.path.join(texturepath, scene_name+'.xml')
	...
	if order=='wrap':
		...
		reorder = np.reshape(reorder, grid_h * grid_w).tolist()
		print(len(reorder))
	...
	if do_fullres:
		...
		texture_files = generate_large_texture(image_files, warpedpath, contactimg_file, reorder, skip)
	else:
		texture_files = generate_contact_sheet(image_files, reorder, skip)

	# export camera center coordinates    
	write_xml_scene(xmlfile, texture_files, image_files, camresults, reorder, skip, do_inverty)
