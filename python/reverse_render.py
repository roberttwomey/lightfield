...
def readSnapshotParams(snapshotfile):
    ...
    infile = file(snapshotfile, 'r')
    lines = infile.readlines()
    
    texture = lines[0].strip('\r\n')
    fscale = lines[1].strip('\r\n')
    ...
    return texture, fscale, roll, ap_loc, ap_size, zoom
...
if __name__ == '__main__':
    ...
    parser.add_argument('infile', help='text file describing refocus parameters')
    ...
    texture, focus, roll, ap_loc, ap_size, zoom = readSnapshotParams(snapshotfile)
    
    # print texture, focus, roll, ap_loc, ap_size, zoom
    scenename = os.path.basename(texture).split('_tex')[0]
    lfdata = os.path.join(datapath, scenename)
    
    warped = os.path.join(lfdata, 'warped')
    output = outfile
