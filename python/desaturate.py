...
def desaturate(img, percent = 0.8):
    ...
    channels = cv2.split(hsb)

    # channels[0] = cv2.equalizeHist(channels[0])
    channels[1] *= percent
    channels[2] = cv2.equalizeHist(channels[2])
    
    hsb = cv2.merge(channels);

    newimg = cv2.cvtColor(hsb, cv2.COLOR_HSV2BGR);
...
if __name__ == '__main__':
    ...
    parser.add_argument('--saturation', default=0.2, type=float, help='saturation factor (0.0 - 1.0)')
    ...
    eqimg = desaturate(img, saturation)
    
    fname, ext = os.path.splitext(os.path.basename(imgfile))
    eqfile = os.path.join(outpath, fname+"_de"+ext.lower())
    cv2.imwrite(eqfile, eqimg)
