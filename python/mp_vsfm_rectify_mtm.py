...
def mp_warp(srcs, params, R_avg, C_avg, nprocs=8):
    ...
    print "out of the process creation loop"

    # Collect all results into a single result dict. We know how many dicts
    # with results to expect.
    resultdict = {}
    for i in range(nprocs):
        print "updating result dictionary for process:", i
        resultdict.update(out_q.get())

    print "retrieved outputs, waitting for processes to join..."
...
def generate_laptop_texture(files, img_path, contactimg_file, reorder = None, skip = []):
    """Generate single texture at less than 512MB(* 2) total"""
    global max_texture_size, grid_h, grid_w, num_textures, thumb_w, thumb_h, x_tiles, y_tiles, x_imgs_per_tile, y_imgs_per_tile
    ...
    for y_img in range(y_imgs_per_tile):
        for x_img in range(x_imgs_per_tile):
            ...
            if cam_pos[0] < grid_w and cam_pos[1] < grid_h:
                ...
                if reorder != None:
                  num = reorder[num]
                #   print "reordered num {0}".format(num)

                # print "Camera", cam_pos, "(image",num,")",
                sys.stdout.write(".")
                if num not in skip:
                    this_img_str = img_str.format(num+1)            # mtm added +1, because the image sources don't begin at image0000, rather image0001
                                                                    # note, when writing the warped images, should prob name in the order this expects,
                                                                    # if using the iterator at the filename
                    img_file = os.path.join(img_path, this_img_str)
                    print img_file
...
def write_xml_scene(camerapos, texture_files, image_files, camresults, reorder = None, skip = [], do_inverty=False):
    ...
    out_f.write("<maxscale>100</maxscale>\n")
    out_f.write("\n")
...
if __name__ == '__main__':
    ...
    texroot = "/Users/admin/Documents/Rover/photogrammetry_alignment/textures"
    ...
    # grid
    print "grid specified: {}".format(gridstr)
    ...
    print "\nparsed camresults: R_avg", R_avg, "C_avg", C_avg, "\n"

    # rectify images
    if do_warp:
        results = mp_warp(image_files, camresults, R_avg, C_avg, 8)
        print "performed the WARP"
    ...
    if order=='wrap':
        ...
        for i in reorder:
            ...
            fname = "image{0:04d}.jpg".format(i)        # mtm changed
