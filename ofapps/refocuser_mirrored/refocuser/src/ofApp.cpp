...
void ofApp::draw(){
    ...
    if(bDebug == true) {
        ...
        ofDrawBitmapString("fade:    \t"+ofToString(fade), 0, 75);
...
void ofApp::keyPressed(int key){
    ...
    if(key=='b') {
        // zoom
        zoom = ofMap(mouseX, 0, ofGetWindowWidth(), 3.0, 0.01);
    }
    if(key=='z') {
        // parallax
    ...
    if(key == 'v') {
        // offsets
        xoffset = ofMap(mouseX, 0, ofGetWindowWidth(), -subwidth*2, subwidth*2);
        yoffset = ofMap(mouseY, 0, ofGetWindowHeight(), -subheight*2, subheight*2);
    }
    if(key == 'c')
        // focus
    ...
    if(key == 'n')
        // fade in (1) out (0)
        fade = ofMap(mouseY, 0, ofGetWindowHeight(), 0, 1);
