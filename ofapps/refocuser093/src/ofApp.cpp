...
void ofApp::setup(){
    ...
    loadDirectory("textures");
    
    ofLog(OF_LOG_VERBOSE, "found " + ofToString(scenefiles.size()) + " scenes in textures directory");
...
void ofApp::draw(){
    ...
    if(bFill) {
        ...
        height = width / subwidth * subheight;
        xoff=0;
        yoff = (ofGetWindowHeight() - height)/2;
    }
//    if(bFill) {
//        height =
//    }
    ...
    // draw fused image
    fbo->draw(xoff, yoff, width, height);
...
//        ofDrawBitmapString("tilenum:  \t"+ofToString(tilenum), 0, -15);
        ofDrawBitmapString("scene:    \t"+ofToString(scenenum), 0, -30);
...
void ofApp::loadDirectory(string path) {
    ...
    // iterate and add to list of scenefiles
    for(int i = 0; i < (int)dir.size(); i++){
        scenefiles.push_back(dir.getPath(i));
    }
}
...
void ofApp::loadNewScene(int newscenenum) {
    
    ofLog(OF_LOG_VERBOSE, ofToString(newscenenum));
    
    int toload = newscenenum;
    if(newscenenum >= scenefiles.size()) toload = 0;
    if(newscenenum < 0) toload = (scenefiles.size() - 1);
    
    ofLog(OF_LOG_VERBOSE, "loading scene " + ofToString(toload) + " of " + ofToString(scenefiles.size()));
          
    string scenefile = scenefiles[toload];
...
void ofApp::loadNewScene(string scenefile) {
...
//        if(std::find(scenefiles.begin(), scenefiles.end(), scenefile) != scenefiles.end()) {
//            
//        } else {
...
void ofApp::snapshot() {
    ...
    snapcount = 0;
    while(!done) {
        ofFile file(scenefiles[scenenum]);
